...
_ACTIVE: ContextVar[Profile | None] = ContextVar("sensei_active_profile", default=None)


def current() -> Profile:
    p = _ACTIVE.get()
    return p if p is not None else _default_profile()
...
def profile_key(profile: Profile | None = None) -> str:
    p = profile or current()
    return f"{p.name}@{p.resolved_home}"


def cache_key(*parts: Any, profile: Profile | None = None) -> tuple:
    return (profile_key(profile), *parts)


def scoped_env(profile: Profile | None = None) -> dict[str, str]:
    p = profile or current()
...
def allowlist_dir(profile: Profile | None = None) -> Path:
    p = profile or current()
...
def config_value(
    *keys: str, default: Any = None, profile: Profile | None = None
) -> Any:

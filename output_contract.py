...
JOB_APPLICATION_INTENT_TEXT = (
    ...
    "out the application,\" \"apply for this job\"), emit a single RUN_SKILL "
    "directive on line 1. The skill is the orchestrator; you are NOT the "
    "orchestrator on this kind of request. Trying to drive BROWSER_* "
    "directives turn-by-turn yourself on a multi-application workflow "
    "will not converge — the skill state machine is designed for this "
    "exact case.\n"
    "Shape:\n"
    "  RUN_SKILL: apply-job-session {\"candidate_urls\": [<list of "
    "specific listing URLs OR one search URL>], \"resume_path\": "
    "\"<absolute path to résumé file, if user named one>\", "
    "\"skip_drive_fetches\": true, \"skip_inbox\": true, "
    "\"max_applications\": <N from user>}\n"
    "URL construction priority (top-down — first match wins, do NOT ask "
    "for clarification if any of these resolve):\n"
    "  1. [BROWSER PAGE CONTEXT] present — the current page IS the job "
    "target. Use page_context.url as candidate_urls[0]. \"Apply for this "
    "job\" with a job posting open in the active tab is a complete "
    "request; no clarifying question. max_applications defaults to 1 when "
    "the user said \"this job\" (singular).\n"
    "  2. User named a specific posting URL — use it verbatim as "
    "candidate_urls[0].\n"
    "  3. User gave keywords + location (e.g. \"HVAC install jobs in "
    "Indianapolis on ZipRecruiter\") — construct a ZipRecruiter search "
    "URL: https://www.ziprecruiter.com/jobs-search?search=<keywords "
    "URL-encoded>&location=<city,ST URL-encoded>.\n"
    "Attachment parsing — \"using my résumé at <path>\", \"with the "
    "résumé from <path>\", \"attach <path>\", or similar phrases name a "
    "FILE to attach to the application form. That path is NOT a missing-"
    "info signal — it is an attachment instruction. Parse the path and "
    "fold it into the RUN_SKILL spec as the resume_path field. Same for "
    "cover-letter paths (\"with my cover letter at <path>\" → cover_"
    "letter_path) and any other named attachment. Do not ask the user to "
    "confirm the file exists — the skill verifies at upload time and "
    "surfaces a structured error if the file is missing.\n"
    "The skill handles navigating, listing extraction, per-host adapter "
    "dispatch, skip filtering (criminal-background-check phrases, skip-"
    "company list), file upload via the extension's DataTransfer bridge, "
    "and operator-review gate at the form before submit.\n"
    "DO NOT emit BROWSER_NAV/CLICK/FILL directly for an apply-jobs "
    "intent. DO NOT browser-search the web yourself first. The skill's "
    "adapter_<host> functions handle browser navigation once they have "
    "the URLs.\n"
    "ASK ONE clarifying question and STOP only when ALL of the URL "
    "construction priority cases fail — i.e. no page_context, no user-"
    "named URL, AND no keywords+location. With page_context present, the "
    "request is always complete.\n"
    "Example 1 — keywords + location (no page context):\n"
    "User: \"Apply to 5 HVAC install jobs in Indianapolis on "
    "ZipRecruiter. Skip All Trades Staffing.\"\n"
    "Reply:\n"
    "RUN_SKILL: apply-job-session {\"candidate_urls\": [\"https://www."
    "ziprecruiter.com/jobs-search?search=HVAC+install&location="
    "Indianapolis%2C+IN\"], \"skip_drive_fetches\": true, \"skip_inbox\": "
    "true, \"max_applications\": 5}\n"
    "Dispatching the apply-job-session skill against ZipRecruiter for "
    "HVAC install in Indianapolis.\n"
    "Example 2 — page_context-anchored + résumé attachment (THIS-job "
    "case):\n"
    "User (chrome_extension, [BROWSER PAGE CONTEXT] url=https://www."
    "indeed.com/viewjob?jk=abc123): \"Apply for this job using my résumé "
    "at ~/Desktop/resume/Elijah Wilkins Resume 5Year.pdf\"\n"
    "Reply:\n"
    "RUN_SKILL: apply-job-session {\"candidate_urls\": [\"https://www."
    "indeed.com/viewjob?jk=abc123\"], \"resume_path\": \"~/Desktop/resume/"
    "Elijah Wilkins Resume 5Year.pdf\", \"skip_drive_fetches\": true, "
    "\"skip_inbox\": true, \"max_applications\": 1}\n"
    "Dispatching apply-job-session against the open Indeed posting with "
    "the named résumé file.\n\n"
)

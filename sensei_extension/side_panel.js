...
async function dispatchMcpAction(action) {
  ...
  try {
    ...
    if (kind === "BROWSER_JS") {
      ...
      // 2026-09-06: js_eval (the MCP tool) sends raw code as `target` with no
      // `command` — that never matched any of the 5 whitelisted commands
      // below and fell straight through to "unknown command: " on every
      // call. Raw code with no whitelisted command now goes through real
      // CDP Runtime.evaluate (mirrors BROWSER_GET_DOM's proven-reliable
      // path) instead of the constrained executeScript runner.
      const rawCode = String(action.target || action.code || "").trim();
      if (!command && rawCode) {
        try {
          const response = await chrome.runtime.sendMessage({
            type: "SENSEI_BROWSER_EVAL",
            tabId: tab.id,
            code: rawCode,
          });
          if (!response?.ok) return { ok: false, error: response?.error || "eval failed" };
          return { ok: true, result: response.result };
        } catch (err) {
          return { ok: false, error: `BROWSER_JS eval dispatch failed: ${err?.message || err}` };
        }
      }

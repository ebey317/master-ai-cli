...
// response at all (confirmed live 2026-09-06: mcp__sensei__tab_list timed
// out twice in a row with the panel closed — root cause traced to mcpPoll's
// setInterval only running in the side panel's document). The side panel
// opens a long-lived port named "sensei-panel" on load; this poller backs
// off entirely whenever that port is connected, so the two never race on
// the same queue.
...
async function _swDispatchMcpAction(action) {
  ...
  try {
    ...
    if (kind === "BROWSER_FIND") {
      // Full parity with dispatchMcpAction's BROWSER_FIND, not just the
      // generic content-script forward — this used to be the one case where
      // the panel-closed path was strictly weaker. Same two-step approach:
      // exact/substring text match via content_script.js first, semantic
      // (LLM-scored) match over the AX tree if that comes up empty or the
...
// CORRECTION (2026-09-06, same day as the first version): live-tested with
// the panel closed and it did NOT work — confirmed zero /extension/pending
// hits from this worker over a real 5s window. Root cause: a plain
// setInterval() does not survive MV3 service-worker idle suspension (~30s
// with no recognized activity) — once Chrome tears the worker down, the
// timer is gone with it, full stop, and nothing brings it back except a
// real Chrome-recognized wake event. The "call a cheap chrome.* API every
// tick to reset the idle clock" idea assumed setInterval callbacks keep
// firing indefinitely in the background; they don't outlive the worker
// they're scheduled in. chrome.alarms is the one primitive Chrome
// guarantees will wake a terminated worker back up, so it has to be the
// PRIMARY mechanism here, not just a backstop — the previous version had
// it as a 1-min-period backstop behind a setInterval that was doing
// nothing once the worker died, which is exactly why every call still
// timed out against the MCP client's 30s wait.
//
// Trade-off, stated plainly: chrome.alarms' minimum period is 1 minute
// (enforced by Chrome itself, not something this code can shrink) — this
// setInterval stays in as a free bonus for whenever the worker happens to
// already be awake for some other reason, but the only cadence this
// implementation can actually GUARANTEE while the panel is closed is
// once per minute. sensei_mcp_server.py's per-tool wait was bumped past
// 60s specifically so a call has a real chance of landing inside that
// window instead of always timing out one tick early. This means the
// panel-closed path is real but slower (up to ~60s), not instant like
// the panel-open path — genuinely different from claude-in-chrome's
// always-fast dispatch, and stated as such rather than implied otherwise.
setInterval(() => {

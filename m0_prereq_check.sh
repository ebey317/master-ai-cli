...
if grep -q "api_handle(payload)" ~/scripts/stt_server.py; then
  green "/chat uses api_handle() wrapper — M0a upgrade present"
elif awk '/self.path == '\''\/chat'\''/{in_chat=1} in_chat && /api\/generate/{found=1} in_chat && /POST \/mode/{in_chat=0} END{exit found ? 0 : 1}' ~/scripts/stt_server.py; then
...
hdr "4) CORS state"
cors=$(grep -E "Access-Control-Allow-Origin" ~/scripts/stt_server.py | head -3 || true)
...
if grep -nq "llava" ~/scripts/prewarm_master_ai.py 2>/dev/null; then
  warn "llava is pinned in prewarm_master_ai.py — remove this line"
  grep -n "llava" ~/scripts/prewarm_master_ai.py
else
...
grep -nE 'llava.*keep_alive|keep_alive.*llava|"keep_alive"\s*:\s*"30m"' ~/scripts/master_ai.py | head -10 || warn "no explicit LLaVA keep_alive found"
echo

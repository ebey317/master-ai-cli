...
def parse_extension_directive(user_input: str) -> tuple[str, dict]:
    ...
    if action == "click":
        ...
        if not selector:
            raise ValueError(
                "click: need a selector (e.g., 'extension click button.submit')"
            )
    ...
    if action == "fill":
        ...
        if " with " not in rest:
            raise ValueError(
                "fill: use format 'extension fill <selector> with <value>'"
            )
...
def main():
    ...
    parser.add_argument(
        "--register",
        choices=["cli", "extension", "pupil"],
        help="Register this surface",
    )

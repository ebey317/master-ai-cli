...
def validate_tool_schema(tool) -> list:
    ...
    if not isinstance(name, str) or not name.strip():
        ...
        problems.append(
            f"invalid tool name {name!r} (allowed: letters, digits, _-, max 64)"
        )
    ...
    if not isinstance(schema, dict):
        ...
        if schema.get("type") != "object":
            problems.append(
                f"tool {name!r}: inputSchema.type must be 'object', got {schema.get('type')!r}"
            )
        ...
        if req is not None and not (
            ...
            problems.append(
                f"tool {name!r}: inputSchema.required must be a list of strings"
            )
    return problems
...
def validate_tools(tools) -> dict:
    ...
    if not isinstance(tools, list) or not tools:
        return {
            "ok": False,
            "problems": ["tools/list returned no tools"],
            "tool_names": [],
        }
    problems, names = [], []
    for t in tools:
...
class StdioMcpClient:
    ...
    def _rpc(
        ...
        return {
            "error": {
                "code": -2,
                "message": f"timeout waiting for {method} response ({timeout}s)",
            }
        }
...
def add_server(name: str, target: str, transport: str = "") -> dict:
    ...
    if not _NAME_RE.match(name):
        return {
            "ok": False,
            "message": f"invalid name {name!r} (letters, digits, _ - only, max 32)",
            "entry": None,
        }
    if not target:
    ...
    if t and t not in VALID_TRANSPORTS:
        return {
            "ok": False,
            "message": f"transport must be stdio or sse, got {transport!r}",
            "entry": None,
        }
    if not t:
    ...
    if t == "sse":
        if not re.match(r"^https?://", target):
            return {
                "ok": False,
                "message": "sse transport needs an http(s) URL",
                "entry": None,
            }
    ...
    if name in cat["servers"]:
        return {
            "ok": False,
            "message": f"server {name!r} already exists (remove it first)",
            "entry": None,
        }
    ...
    if probe["valid"]:
        msg = (
            f"added {name} ({t}, {len(probe['tool_names'])} tools validated) — enabled\n"
            f"  tools: {', '.join(probe['tool_names'])}"
        )
        ok = True
    else:
        msg = (
            f"registered {name} but LEFT DISABLED — probe failed:\n"
...
def revalidate(name: str) -> dict:
    ...
    if probe["valid"]:
        return {
            "ok": True,
            "message": f"{name}: valid — {len(probe['tool_names'])} tools: {', '.join(probe['tool_names'])}",
        }
    return {
        "ok": False,
        "message": f"{name}: INVALID — " + "; ".join(probe["problems"][:4]),
    }
...
def format_catalog(G: str, R: str, Y: str, C: str, W: str, D: str, X: str) -> str:
    ...
    lines.append(
        f"  {D}usage: mcp add <name> <command|url> [--transport stdio|sse] · mcp remove|enable|disable|validate <name> · mcp tools <name>{X}"
    )
    return "\n".join(lines)


def format_tools(
    name: str, G: str, R: str, Y: str, C: str, W: str, D: str, X: str
) -> str:
    s = get_server(name)
    if not s:
